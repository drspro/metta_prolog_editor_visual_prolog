...
ask_trace_on():-
   trace,
   dlg_ask("Turn trace on?",["Yes","No"],_Res), !.
   %dlg_Note("Set to ",Res),
   %   Res = "Yes", !, 
...
match_all_komma102_add_atom( Outvar_e2 ):-
  komma102_sudoku_puzzle_state102_cell_candidates( R , C , Q , State , Outvx1 ) ,  
  match_all3( "&self" , Outvx1 , Outvar_e2 ) ,  
  ! .
...
match_all_sudoku_puzzle_state93( R , Outvar_e2 ):-
  ...
  match_all( "&self" , Outv_not_db , State , Outvx1 , Outvar_e2 ) ,  
  ! .
...
match_sudoku_puzzle_state92( Q , R , C , State , Outvar_e2 ):-
  ...
  match( "&self" , Outv_not_db , Q , Outvx1 , Outvar_e2 ) ,  
  ! .
