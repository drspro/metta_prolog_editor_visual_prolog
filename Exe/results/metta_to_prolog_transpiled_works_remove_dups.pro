...
groupe( _ , _ , _ , Tail_new ,  Tail_new   ):- ! .
...
condition_if_check_is_equal_let_star( List_netto , Expr , List_netto ):-
  check_is_equal( Expr , "empty_atom" , Outvx1 ) ,  
  Outvx1 = "True" ,  
  ! .
%Clause_implementation--  part2  condition_if_check_is_equal_let_star
condition_if_check_is_equal_let_star( List_netto , Expr , Outvar_e3 ):-
  let_star_groupe( List_nettx , Expr , Head , Tail , List_netto , Tail_new , Outvx2 ) ,  
  Outvar_e3 = Outvx2 ,  
  ! .
...
contains_symbol_cdr_atom( Sym , List , Outvar_e2 ):-
  ...
  ! .
%Subclausex  :  groupe_car_atom_cdr_atom_add_elem_if_not_in_listx_remove_dups
groupe_car_atom_cdr_atom_add_elem_if_not_in_listx_remove_dups( Expr , Head , Tail , List_netto , List_nettx , Tail_new , Outvar_e5 ):-
  car_atom( Expr , Head ) ,  
  cdr_atom( Expr , Tail ) ,  
  add_elem_if_not_in_listx( List_netto , Head , List_nettx ) ,  
  remove_dups( List_nettx , Tail , Tail_new ) ,  
  groupe( Head , Tail , List_nettx , Tail_new , Outvar_e5 ) ,  
  ! .
%Subclausex  :  let_star_groupe
let_star_groupe( List_nettx , Expr , Head , Tail , List_netto , Tail_new , Outvar_e2 ):-
  groupe_car_atom_cdr_atom_add_elem_if_not_in_listx_remove_dups( Expr , Head , Tail , List_netto , List_nettx , Tail_new , Outvx1 ) ,  
  let_star( Outvx1 , List_nettx , Outvar_e2 ) ,  
  ! .

%end_result remove_dups
remove_dups( List_netto , Expr , Outp0 ):-
  condition_if_check_is_equal_let_star( List_netto , Expr , Outp0 ) ,  
  ! .
